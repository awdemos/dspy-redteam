...
{:else if isDesktop.current}
	...
	>
		...
		{#if backgroundImageExists}
			<div
				class="absolute top-0 right-0 bottom-0 left-[650px] z-0 m-6 overflow-hidden rounded-[40px] bg-[#0a0a0a] 2xl:left-[800px]"
			>
				<img
					src={cachedBackgroundImage.getUrl()}
					class="{cn(
						animate && 'animate-bg-zoom'
					)} h-screen object-cover w-[calc(100vw-650px)] 2xl:w-[calc(100vw-800px)] opacity-40"
					alt={m.login_background()}
				/>
				{#if rightPanel}
					<div class="absolute inset-0 z-10 flex items-center justify-center bg-[#0a0a0a]/60 p-12"
					>
						{@render rightPanel()}
					</div>
				{/if}
