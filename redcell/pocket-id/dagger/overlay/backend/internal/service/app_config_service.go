...
func (s *AppConfigService) getDefaultDbConfig() *model.AppConfig {
	...
	return &model.AppConfig{
		// General
		AppName:                   model.AppConfigVariable{Value: "Redcell"},
		...
		AllowUserSignups:          model.AppConfigVariable{Value: "open"},
		SignupDefaultUserGroupIDs: model.AppConfigVariable{Value: "[]"},
		SignupDefaultCustomClaims: model.AppConfigVariable{Value: "[]"},
		AccentColor:               model.AppConfigVariable{Value: "#f59e0b"},
		// Internal
		InstanceID: model.AppConfigVariable{Value: ""},
		// Email
		RequireUserEmail:              model.AppConfigVariable{Value: "true"},
		SmtpHost:                      model.AppConfigVariable{Value: getEnvDefault("SMTP_HOST", "")},
		SmtpPort:                      model.AppConfigVariable{Value: getEnvDefault("SMTP_PORT", "")},
		SmtpFrom:                      model.AppConfigVariable{Value: getEnvDefault("SMTP_FROM", "")},
		SmtpUser:                      model.AppConfigVariable{Value: getEnvDefault("SMTP_USER", "")},
		SmtpPassword:                  model.AppConfigVariable{Value: getEnvDefault("SMTP_PASSWORD", "")},
		SmtpTls:                       model.AppConfigVariable{Value: getEnvDefault("SMTP_TLS", "none")},
		SmtpSkipCertVerify:            model.AppConfigVariable{Value: getEnvDefault("SMTP_SKIP_CERT_VERIFY", "false")},
		EmailLoginNotificationEnabled: model.AppConfigVariable{Value: "false"},
		EmailOneTimeAccessAsUnauthenticatedEnabled: model.AppConfigVariable{Value: "true"},
...
func (s *AppConfigService) LoadDbConfig(ctx context.Context) (err error) {
	...
	// Redcell: signups must remain open so the email-code login flow can
	// create first-time users. If the database has a different value (e.g. from an
	// older bootstrap), force it back to the default and persist the override.
	slog.Info("redcell: loaded db config", "allowUserSignups", dest.AllowUserSignups.Value)
	if dest.AllowUserSignups.Value != "open" {
		dest.AllowUserSignups.Value = "open"
		if err := s.UpdateAppConfigValues(ctx, "allowUserSignups", "open"); err != nil {
			return fmt.Errorf("failed to enforce open signups in database: %w", err)
		}
		slog.Info("redcell: enforced allowUserSignups=open in database")
	}
