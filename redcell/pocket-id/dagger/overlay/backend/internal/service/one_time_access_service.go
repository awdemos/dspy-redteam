...
// RequestOneTimeAccessEmailAsUnauthenticatedUser sends a one-time login code to the
// provided email address. If signups are enabled and the user does not yet exist,
// a new user is created first so first-time sign-ins work for the KV Cache Store flow.
func (s *OneTimeAccessService) RequestOneTimeAccessEmailAsUnauthenticatedUser(ctx context.Context, userEmail, redirectPath string) (string, error) {
	cfg := s.appConfigService.GetDbConfig()
	slog.Info("kvcachestore: one-time access request", "email", userEmail, "enabled", cfg.EmailOneTimeAccessAsUnauthenticatedEnabled.Value, "signups", cfg.AllowUserSignups.Value)

	isDisabled := !cfg.EmailOneTimeAccessAsUnauthenticatedEnabled.IsTrue()
	if isDisabled {
		return "", &common.OneTimeAccessDisabledError{}
	}

	var userId string
	err := s.db.Model(&model.User{}).Select("id").Where("email = ?", userEmail).First(&userId).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		slog.Info("kvcachestore: user not found for one-time access", "email", userEmail, "signups", cfg.AllowUserSignups.Value)
		// KV Cache Store: if signups are open, auto-create the user so they can receive a login code.
		// Otherwise keep the upstream behavior of silently returning 204 to prevent
		// email enumeration.
		...
		emailCopy := strings.ToLower(strings.TrimSpace(userEmail))
		username := strings.Split(emailCopy, "@")[0]
		slog.Info("kvcachestore: auto-creating user for one-time access", "email", emailCopy, "username", username)
		newUser, createErr := s.userService.CreateUser(ctx, dto.UserCreateDto{
			Username:      username,
			Email:         &emailCopy,
			EmailVerified: true,
		})
		if createErr != nil {
			slog.Error("kvcachestore: failed to auto-create user", "email", emailCopy, "error", createErr)
			return "", createErr
		}
		slog.Info("kvcachestore: auto-created user", "id", newUser.ID, "email", emailCopy)
		userId = newUser.ID
	} else if err != nil {
		return "", err
	}
...
func (s *OneTimeAccessService) requestOneTimeAccessEmailInternal(ctx context.Context, userID, redirectPath string, ttl time.Duration, withDeviceToken bool) (*string, error) {
	...
	go func() {
		...
		slog.InfoContext(innerCtx, "kvcachestore: sending one-time access email", "email", *user.Email, "token", oneTimeAccessToken)
		...
		if errInternal != nil {
			slog.ErrorContext(innerCtx, "kvcachestore: failed to send one-time access email", slog.Any("error", errInternal), slog.String("address", *user.Email))
			return
		}
		slog.InfoContext(innerCtx, "kvcachestore: one-time access email sent", "email", *user.Email)
	}()
