...
func initServices(ctx context.Context, db *gorm.DB, httpClient *http.Client, imageExtensions map[string]string, fileStorage storage.FileStorage, scheduler *job.Scheduler) (svc *services, err error) {
	...
	// Redcell: make sure the Redcell OIDC client always uses the Redcell
	// callback URL, regardless of what a previous bootstrap wrote to the database.
	if err := enforcePortalCallbackURL(ctx, db); err != nil {
		return nil, fmt.Errorf("failed to enforce portal callback URL: %w", err)
	}

	return svc, nil
}

func enforcePortalCallbackURL(ctx context.Context, db *gorm.DB) error {
	const clientID = "redcell"
	desired := model.UrlList{"https://redcells.net/auth/callback"}

	var client model.OidcClient
	err := db.WithContext(ctx).First(&client, "id = ?", clientID).Error
	if err != nil {
	...
	if reflect.DeepEqual(client.CallbackURLs, desired) {
		return nil
	}

	client.CallbackURLs = desired
