...
func initApplicationImages(ctx context.Context, fileStorage storage.FileStorage) (map[string]string, error) {
	...
	// Redcell customization: always re-seed application images from the
	// embedded resources on startup so image and branding updates ship with the
	// image rather than being locked behind the persisted .inited marker.
	initedPath := path.Join("application-images", ".inited")
	if _, _, err := fileStorage.Open(ctx, initedPath); err == nil {
		if err := fileStorage.Delete(ctx, initedPath); err != nil {
			return nil, fmt.Errorf("failed to remove old .inited marker: %w", err)
		}
		// Also remove any previously seeded images so the new embedded versions take effect.
		for _, f := range destinationFiles {
			_, name := path.Split(f.Path)
			if name == "" || name == ".inited" {
				continue
			...
			if err := fileStorage.Delete(ctx, f.Path); err != nil {
				return nil, fmt.Errorf("failed to remove old application image '%s': %w", name, err)
			}
		}
		dstNameToExt = make(map[string]string)
